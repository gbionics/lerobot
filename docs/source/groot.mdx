...
accelerate launch \
  ...
  --logger.type=wandb \
  --logger.disable_artifact=true \
  --job_name=$JOB_NAME
```

LeRobot also supports built-in MLflow logging. Use
`--logger.type=mlflow` and optional `--logger.experiment_name=<name>` if
you want MLflow instead of W&B.

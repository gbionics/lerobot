...
- A successful experiment is one where the human has to intervene at the start but then reduces the amount of interventions as the policy improves. You can monitor the intervention rate in the `wandb` dashboard. If you prefer MLflow, LeRobot also provides a built-in backend that you can enable with `--logger.type=mlflow`.
...
If you set `--logger.type=wandb` in your configuration, you can monitor training progress in real-time through the [Weights & Biases](https://wandb.ai/site/) dashboard. If you use MLflow instead, set `--logger.type=mlflow` and monitor the run in your MLflow UI.

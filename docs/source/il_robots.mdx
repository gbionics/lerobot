...
4. We provided `logger.type=wandb` to use [Weights and Biases](https://docs.wandb.ai/quickstart) for visualizing training plots. This is optional but if you use it, make sure you are logged in by running `wandb login`.
5. You can also use LeRobot's built-in MLflow backend instead of W&B by passing
   `--logger.type=mlflow` (and optionally `--logger.experiment_name=<name>`).

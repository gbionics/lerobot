...
You can use MLflow instead of W&B by replacing the W&B flag with
`--logger.type=mlflow` and optional
`--logger.experiment_name=<your_experiment>`.
...
4. **Single process logging**: Only the main process logs to the selected
   logger backend (W&B or MLflow) and saves checkpoints

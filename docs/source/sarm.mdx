...
W&B logging is optional in the examples above. LeRobot also has built-in
MLflow support: use `--logger.type=mlflow` (and optionally
`--logger.experiment_name=<name>`) if you prefer MLflow.
